# -*- coding: utf-8 -*-
"""
This code implements the training, validation, and testing of GMFE-Net on the SensatUrban dataset.
...
tree_path =os.path.join(BASE_DIR, 'random_20.000')
...
def get_batch_gen(split):    # Generate data for training, validation, or testing
    if split == 'training':
...
def GMFE_Unit(pts, fts_prev, qrs,rgb,rgb_qrs, K, D, P, C, with_local, bn_decay=None):
    ...
    '''
    ########### Ablation (replace attention-pooling block with max-pooling) ############
    d_in=pts_X_0.shape[3]
    f_agg=tf.reduce_max(pts_X_0,axis=-2)
    '''
    
    ######## Global multi-feature fusion and extraction (GloMFE) block ########
    local_volume = tf.math.pow(tf.reduce_max(tf.reduce_max(relative_dis, -1), -1), 3)    #(B,N)
    global_dis = tf.sqrt(tf.reduce_sum(tf.square(qrs), axis=-1, keepdims=True))    #(B,N,1)
    global_volume = tf.pow(tf.reduce_max(global_dis, axis=-1), 3)    #(B,N)
    lg_volume_ratio = tf.expand_dims(local_volume / global_volume, -1)    #(B,N,1)
...
def get_model(layer_pts,layer_rgb, sconv_params, sdconv_params, fc_params, weight_decay=0.0, bn_decay=None, num_classes=num_classes):
    ...
    '''
    ##################### Ablation (repalce GMFE-Unit in the decoder stage) ##########
    f_decoder_list = []
    fts=layer_fts_list[-1]
    for i in range(len(layer_pts_list)-1):
    # i=0
        down_pts=layer_pts_list[-i-1]
        up_pts=layer_pts_list[-i-2]
        upsample_idx=knn_indices_general(up_pts, down_pts, 1)    #中心点在前，仓库点在后
        # down_fts=layer_fts_list[-i-1]
        up_fts_gather=tf.gather_nd(fts, upsample_idx)
        up_fts_gather=tf.squeeze(up_fts_gather)
        up_fts=layer_fts_list[-i-2]
        if up_fts==None:
            fts_concat=up_fts_gather
            c_channel=up_fts_gather.shape[-1]//2
        else:
            fts_concat=tf.concat([up_fts,up_fts_gather],axis=-1)
            c_channel=up_fts.shape[-1]
        # print(c_channel)
        fts_decoder=conv1dtranspose(fts_concat,c_channel)
        fts=fts_decoder
        f_decoder_list.append(fts_decoder)
    
    layer_fts=f_decoder_list[-1]
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    for idx in range(len(xbeg_list)): 
       ...
       cond = xcond & ycond    
       if np.sum(cond) < num_points//16:    # Discards point clouds with less than a certain number
           continue
...
if press_key=='1':
    ...
    checkpoint_filepath = os.path.join(BASE_DIR,'weights\\check_point_202566')
    ...
    checkpoint_filepath = os.path.join(BASE_DIR,'weights\\check_point_20241125')
    GMFE_Net.load_weights(checkpoint_filepath)
